...
export default function ElecteurDashboard() {
  ...
  const canVote       = (e) => e.statut === "EN_COURS" && !e.a_vote;
  const canSeeResults = (e) => e.statut === "TERMINEE";

  const stats = {
    total:     elections.length,
    enCours:   elections.filter(e => e.statut === "EN_COURS").length,
    votes:     elections.filter(e => e.a_vote).length,
    terminees: elections.filter(e => e.statut === "TERMINEE").length,
  };
                  ...
                  <li key={election.id_election} className="px-6 py-5 flex items-center justify-between hover:bg-indigo-50/40 transition-all">
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      ...
                      <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 text-lg
                        ${election.statut === "EN_COURS" && !election.a_vote ? "bg-indigo-100 text-indigo-600" :
                          election.a_vote ? "bg-emerald-100 text-emerald-600" :
                          election.statut === "TERMINEE" ? "bg-gray-100 text-gray-400" :
                          "bg-amber-100 text-amber-600"}`}>
                        {election.statut === "EN_COURS" && !election.a_vote ? <FiClock /> :
                         election.a_vote ? <FiCheckCircle /> :
                         election.statut === "TERMINEE" ? <FiLock /> : <FiCalendar />}
                      </div>
                    ...
                    <div className="ml-4 flex-shrink-0">
                      ...
                      {election.a_vote && election.statut === "EN_COURS" && (
