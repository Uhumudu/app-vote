...
export default function VotePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [elecRes, candidatsRes] = await Promise.all([
        api.get(`/electeur/elections/${electionId}`),
        api.get(`/electeur/elections/${electionId}/candidats`),
      ]);
      setElection(elecRes.data);
      setCandidats(candidatsRes.data);
      setTourActuel(elecRes.data.tour_actuel || 1);

      if (elecRes.data.type === "LISTE") {
        const listesRes = await api.get(`/electeur/elections/${electionId}/listes`);
  ...
  const handleVote = async () => {
    ...
    try {
      // ✅ Route différente selon le type de scrutin
      if (election.type === "LISTE") {
        await api.post(`/elections/${electionId}/voter-liste`, {
          liste_id: selected[0],
        });
      } else {
  ...
  if (voted) return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-indigo-200 to-indigo-300 flex items-center justify-center px-4">
      <div className="bg-white rounded-3xl shadow-xl p-10 max-w-md w-full text-center">
        ...
        <p className="text-indigo-700 font-bold text-lg mb-2">{election?.titre}</p>

        {/* Badge tour */}
        <span className="inline-block bg-indigo-100 text-indigo-700 text-xs font-bold px-3 py-1 rounded-full mb-6">
          Tour {tourActuel}
        </span>
        ...
        <div className="bg-indigo-700 rounded-2xl p-6 mb-8 shadow-lg">
          <div className="flex items-start justify-between flex-wrap gap-4">
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              {/* ✅ Badge tour visible uniquement pour les scrutins liste */}
              {isListe && (
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm mt-0.5">
              {isBinominal
                ...
                : isListe
                ? `Sélectionnez une liste pour valider votre vote (tour ${tourActuel}).`
        ...
        {isListe && (
          <div className="space-y-4 mb-8">
            <h2 className="text-sm font-bold text-indigo-400 uppercase tracking-widest mb-4">
              Listes candidates — Tour {tourActuel}
            </h2>
            {listes.length === 0 ? (
              <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center text-gray-400">
                Aucune liste disponible pour ce tour.
              </div>
            ) : (
              listes.map((liste) => {
