...
function ScrutinBadge({ type }) {
  const map = {
    LISTE:      { icon: <FiList size={10} />,      label: "Scrutin de liste",   cls: "bg-violet-100 text-violet-600 border border-violet-200" },
    UNINOMINAL: { icon: <FiUser size={10} />,      label: "Scrutin uninominal", cls: "bg-sky-100 text-sky-600 border border-sky-200" },
    BINOMINAL:  { icon: <FiUserCheck size={10} />, label: "Scrutin binominal",  cls: "bg-teal-100 text-teal-600 border border-teal-200" },
  };
  const s = map[type] || { icon: null, label: type, cls: "bg-gray-100 text-gray-500 border border-gray-200" };
  return (
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-indigo-100 overflow-hidden">
      <SectionHeader
        ...
        subtitle={`${totalVotes} vote${totalVotes > 1 ? "s" : ""} exprimé${totalVotes > 1 ? "s" : ""}`}
      />
          ...
          return (
            <li
              ...
              className={`px-6 py-5 flex items-center gap-4 transition-colors ${
                isWin ? "bg-amber-50/40 border-l-4 border-l-amber-300" : "hover:bg-gray-50/60 border-l-4 border-l-transparent"
              }`}
            >
              <RankBadge index={i} />
              {photoUrl ? (
        ...
        {candidats.map((c, i) => {
          ...
          return (
            <li
              key={c.id_candidat}
              className={`px-6 py-5 flex items-center gap-4 transition-colors ${
                isWin ? "bg-amber-50/40 border-l-4 border-l-amber-300"
                      : isEven ? "bg-teal-50/20 hover:bg-teal-50/40 border-l-4 border-l-teal-200"
...
function SectionListe({ listes, dataTours, totalVotes }) {
  ...
  return (
    <>
      {dataTours?.tours?.length > 0 && (
        <div className="bg-white rounded-2xl shadow-sm border border-indigo-100 overflow-hidden mb-6">
          <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
            ...
            <span className="text-xs bg-indigo-100 text-indigo-600 font-bold px-2.5 py-1 rounded-full border border-indigo-200">
            ...
            <div className="px-6 py-5 bg-amber-50/60 border-t border-amber-100">
              ...
              <div className="flex flex-wrap gap-3">
                {dataTours.sieges.map((s, i) => (
                  <div key={s.liste_id} className={`flex items-center gap-3 rounded-xl px-4 py-2.5 border shadow-sm ${
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-indigo-100 overflow-hidden">
        <SectionHeader
          title="Résultats finaux par liste"
          subtitle={`${totalVotes} vote${totalVotes > 1 ? "s" : ""} exprimé${totalVotes > 1 ? "s" : ""}`}
        />
        {!listes?.length ? (
          <EmptyResults />
...
function SectionHeader({ title, subtitle }) {
  return (
    <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between flex-wrap gap-2 bg-gradient-to-r from-indigo-50/50 to-white">
...
function EmptyResults() {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center px-6">
      <div className="w-12 h-12 bg-indigo-50 rounded-2xl flex items-center justify-center mb-3 border border-indigo-100">
        <FiBarChart2 className="text-indigo-300" size={20} />
      </div>
...
export default function ResultatsElecteur() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-indigo-200 to-indigo-300 flex items-center justify-center">
      <div className="bg-white/80 backdrop-blur rounded-2xl p-10 flex flex-col items-center gap-4 shadow-lg border border-indigo-100">
        <div className="w-12 h-12 border-4 border-indigo-100 border-t-indigo-600 rounded-full animate-spin" />
        <p className="text-indigo-500 font-semibold text-sm">Chargement des résultats…</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-indigo-200 to-indigo-300 flex items-center justify-center">
      <div className="bg-white rounded-2xl p-8 text-center shadow-sm border border-red-100 max-w-sm">
        <div className="w-12 h-12 bg-red-50 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-red-100">
          <FiAward className="text-red-300" size={20} />
        </div>
        <p className="text-red-500 font-bold mb-4">{error}</p>
        <button
          onClick={() => navigate("/DashboardElecteur")}
          className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-semibold text-sm hover:bg-indigo-700 active:scale-95 transition-all shadow-md shadow-indigo-200/60"
        >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-indigo-200 to-indigo-300">
      ...
      <header className="bg-white/90 backdrop-blur-md border-b border-indigo-100 px-6 h-16 flex items-center gap-3 sticky top-0 z-10 shadow-sm">
        <button
          onClick={() => navigate("/DashboardElecteur")}
          className="flex items-center gap-2 text-indigo-600 hover:text-indigo-800 font-semibold text-sm transition-colors hover:bg-indigo-50 px-3 py-1.5 rounded-lg border border-transparent hover:border-indigo-100"
        >
        ...
        <span className="text-[11px] bg-indigo-100 text-indigo-600 px-2.5 py-1 rounded-full font-bold uppercase tracking-wider border border-indigo-200">
          Résultats
        </span>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-6 py-10">
        ...
        {/* ── TITRE ──────────────────────────────────────────────────────── */}
        <div className="bg-indigo-700 rounded-2xl px-6 py-6 mb-8 flex items-center justify-between gap-4 flex-wrap shadow-md shadow-indigo-300/40 ring-1 ring-indigo-600">
          <div>
            <p className="text-indigo-300 text-xs font-bold uppercase tracking-widest mb-1.5">Élection</p>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isListe && nbTours > 1 && (
              <span className="bg-orange-100 text-orange-600 text-xs font-semibold px-2.5 py-1 rounded-full inline-flex items-center gap-1 border border-orange-200">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            { icon: <FiUsers size={16} />,     value: totalElecteurs,          label: "Inscrits",       color: "#4338ca", bg: "bg-white", border: "border-indigo-100",  top: "bg-indigo-500"  },
            { icon: <FiBarChart2 size={16} />, value: totalVotes,              label: "Votes exprimés", color: "#059669", bg: "bg-white", border: "border-emerald-100", top: "bg-emerald-500" },
            { icon: <FiAward size={16} />,     value: `${tauxParticipation}%`, label: "Participation",  color: "#d97706", bg: "bg-white", border: "border-amber-100",   top: "bg-amber-400"   },
          ].map((kpi, i) => (
            <div key={i} className={`${kpi.bg} rounded-2xl border ${kpi.border} overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200`}>
        ...
        <div className="mt-8 text-center">
          <button
            onClick={() => navigate("/DashboardElecteur")}
            className="inline-flex items-center gap-2 px-6 py-3 bg-white border-2 border-indigo-200 text-indigo-700 rounded-xl hover:bg-indigo-50 hover:border-indigo-300 active:scale-95 transition-all font-semibold text-sm shadow-sm"
          >
