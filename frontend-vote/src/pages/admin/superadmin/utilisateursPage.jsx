...
import {
  FiTrash2, FiEdit, FiUserPlus, FiSearch,
...
const ROLE_CONFIG = {
  ...
  CANDIDAT_PUBLIC:         { bg: "#fce7f3", color: "#9d174d", border: "#f9a8d4", label: "Candidat Public" }, // ✅ AJOUTÉ
};
...
export default function UtilisateursPage() {
  ...
  const [roleFilter, setRoleFilter] = useState("TOUS"); // ✅ AJOUTÉ
  ...
  // Filtrage par recherche + rôle
  const filtered = users.filter(u => {
    const matchSearch = `${u.prenom} ${u.nom} ${u.email} ${u.role}`
      .toLowerCase()
  ...
  const stats = {
    ...
    candidats: users.filter(u => u.role === "CANDIDAT_PUBLIC").length, // ✅ AJOUTÉ
  };

  // Filtres rapides (pills cliquables)
  const ROLE_FILTERS = [
    { key: "TOUS",                   label: "Tous",             count: stats.total,     color: "#1d4ed8" },
    { key: "ADMIN_ELECTION",         label: "Admins",           count: stats.admins,    color: "#7c3aed" },
    { key: "ELECTEUR",               label: "Électeurs",        count: stats.electeurs, color: "#059669" },
    { key: "ADMIN_ELECTION_PENDING", label: "En attente",       count: stats.pending,   color: "#d97706" },
    { key: "CANDIDAT_PUBLIC",        label: "Candidats publics",count: stats.candidats, color: "#9d174d" }, // ✅
  ];
        ...
        {!loading && (
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-6">
            {[
              ...
              { label: "Candidats publics", value: stats.candidats, color: "#9d174d" }, // ✅
            ].map((k, i) => (
              <div
                key={i}
                onClick={() => setRoleFilter(ROLE_FILTERS[i].key)}
                className={`bg-white rounded-2xl border p-4 text-center shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer ${
                  roleFilter === ROLE_FILTERS[i].key
                    ? "ring-2 ring-offset-1"
                    : "border-gray-200"
                }`}
                style={roleFilter === ROLE_FILTERS[i].key ? { ringColor: k.color } : {}}
              >
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
            ...
            {ROLE_FILTERS.map(f => (
              ...
              >
                ...
                <span className={`px-1.5 py-0.5 rounded-full text-xs ${
                  roleFilter === f.key ? "bg-white/25" : "bg-gray-100"
                }`}
                  style={roleFilter === f.key ? { color: f.color } : { color: "#6b7280" }}
                >
                  {f.count}
                </span>
              </button>
          ...
          <div className="relative sm:ml-auto">
            ...
            <input
              ...
              className="w-full sm:w-64 pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all"
            />
          </div>
        </div>
              ...
              {loading ? (
                ...
                currentUsers.map((u, index) => {
                  const roleConf = ROLE_CONFIG[u.role] || {
                    bg: "#f3f4f6", color: "#6b7280", border: "#d1d5db", label: u.role
                  };
                  return (
                    <tr key={u.id}
          ...
          {!loading && filtered.length > usersPerPage && (
            <div className="flex items-center justify-between px-6 py-4 bg-gray-50 border-t border-gray-100">
              <p className="text-xs text-gray-500 font-medium">
                Affichage de <span className="font-bold">{indexOfFirstUser + 1}</span> à{" "}
                <span className="font-bold">{Math.min(indexOfLastUser, filtered.length)}</span> sur{" "}
