    ...
    Papa.parse(file, {
      ...
      complete: (result) => {
        ...
        result.data.forEach((row, index) => {
          ...
          preview.push({ nom: row.nom.trim(), prenom: row.prenom.trim(), email: row.email.trim(), actif: row.actif ? row.actif.toLowerCase() === "true" : true });
        });
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {showDropdown && (
                <div className="absolute right-0 mt-2 w-56 bg-white border border-gray-100 rounded-xl shadow-lg z-10 overflow-hidden">
                  ...
                  <button
                    onClick={() => {
                      const csv = "nom,prenom,email,actif\nDupont,Jean,jean.dupont@email.com,true\n";
                      const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
                      const url = URL.createObjectURL(blob);
                      const a = document.createElement("a");
                      a.href = url; a.download = "modele_electeurs.csv";
                      a.click(); URL.revokeObjectURL(url);
