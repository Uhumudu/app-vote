...
import {
  FiEye, FiEyeOff, FiCopy, FiArrowLeft, FiHome,
  FiLogOut, FiUserCheck, FiCalendar, FiSettings, FiRefreshCw,
  FiShield, FiUser, FiLock, FiAlertTriangle, FiMail, FiMenu, FiX
} from "react-icons/fi";
...
export default function AjouterElecteur() {
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-indigo-100 via-indigo-200 to-indigo-300">
      ...
      {/* ── Sidebar desktop (fixe, visible à partir de lg) ── */}
          ...
          <div className="bg-white p-6 sm:p-8 lg:p-10 rounded-3xl shadow-xl w-full max-w-lg lg:flex-shrink-0">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                {/* 2 colonnes sur sm+, 1 colonne sur mobile */}
              ...
              <div>
                ...
                <Field label="Mot de passe provisoire" icon={<FiLock size={13} />}>
                  {/* Rangée mot de passe — flex-wrap sur très petit écran */}
...
//   FiShield, FiUser, FiLock, FiAlertTriangle, FiMail
// } from "react-icons/fi";
// import api from "../../../services/api.jsx";
...
// /* ── Champ avec icône flottante et label visible ── */
// const Field = ({ label, required, icon, children }) => (
//   <div>
//     <label className="block mb-2 font-semibold text-gray-700 text-sm flex items-center gap-1.5">
//       <span className="text-indigo-400">{icon}</span>
...
//         {/* Formulaire */}
//         <div className="bg-white p-10 rounded-3xl shadow-xl w-full max-w-lg flex-shrink-0">
...
//                 <Field label="Nom" required icon={<FiUser size={13} />}>
//                   <input type="text" value={nom} onChange={e => setNom(e.target.value)} required
//                     placeholder="example" className={inputCls} />
//                 </Field>
//                 <Field label="Prénom" required icon={<FiUser size={13} />}>
//                   <input type="text" value={prenom} onChange={e => setPrenom(e.target.value)} required
//                     placeholder="don" className={inputCls} />
//                 </Field>
//               </div>
//             </div>
...
//               <Field label="Adresse e-mail" required icon={<FiMail size={13} />}>
//                 <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
//                   placeholder="example.don@email.com" className={`${inputCls} mb-4`} />
//               </Field>

//               <Field label="Mot de passe provisoire" icon={<FiLock size={13} />}>
//                 <div className="flex items-center gap-2">
//                   <input type={showPassword ? "text" : "password"} value={motDePasse} readOnly
//                     className="flex-1 px-4 py-3.5 border-2 border-gray-300 rounded-xl font-mono text-sm font-medium bg-gray-50 text-gray-700 outline-none" />
//                   <button type="button" onClick={() => setShowPassword(!showPassword)}
//                     className="p-3.5 bg-gray-100 hover:bg-gray-200 border-2 border-gray-200 rounded-xl transition text-gray-500">
//                     {showPassword ? <FiEyeOff size={15} /> : <FiEye size={15} />}
//                   </button>
...
//                         : "bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-700"
//                     }`}>
//                     <FiCopy size={13} />{copySuccess ? "Copié !" : "Copier"}
//                   </button>
//                 </div>
//                 <div className="mt-2 flex items-start gap-2.5 p-3 bg-indigo-50 border-2 border-indigo-200 rounded-xl">
//                   <FiAlertTriangle size={14} className="text-indigo-500 mt-0.5 shrink-0" />
...
//                   </p>
//                 </div>
//                 <FiShield size={16} className={actif ? "text-indigo-400" : "text-gray-300"} />
//               </label>
//             </div>
