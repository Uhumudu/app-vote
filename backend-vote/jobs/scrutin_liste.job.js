...
export const depouilllerTour = async (electionId) => {
  ...
  // ✅ date_fin du prochain tour = maintenant + duree_tour_minutes (en heure locale MySQL)
  const dureeMin = duree_tour_minutes || 1440;
  const dFin     = new Date(Date.now() + dureeMin * 60000);
  const pad      = n => String(n).padStart(2, "0");
  const dateFinTour = (
    `${dFin.getFullYear()}-${pad(dFin.getMonth() + 1)}-${pad(dFin.getDate())}` +
    ` ${pad(dFin.getHours())}:${pad(dFin.getMinutes())}:00`
  );
...
export const checkDepouillementAuto = async () => {
  try {
    // ✅ On récupère TOUTES les élections LISTE EN_COURS (sans filtrer sur date_fin)
    // Le filtrage se fait en JS après vérification/création du tour
    ...
    for (const elec of elections) {
      try {
        const dureeMin = elec.duree_tour_minutes || 1440;

        // ── Vérifier si un tour existe déjà pour cette élection ────────────
        ...
        if (existCount[0].cnt === 0) {
          // ✅ Aucun tour → créer le tour 1 avec date_fin = maintenant + durée
          // (on NE réutilise PAS elec.date_fin qui peut être incorrecte/passée)
          const dFin = new Date(Date.now() + dureeMin * 60000);
          const pad  = n => String(n).padStart(2, "0");
          const dateFinTour1 = (
            `${dFin.getFullYear()}-${pad(dFin.getMonth() + 1)}-${pad(dFin.getDate())}` +
          ...
          await pool.execute(
            `INSERT INTO tour_election (election_id, numero_tour, statut, date_fin_tour)
             VALUES (?, 1, 'EN_COURS', ?)`,
            [elec.id_election, dateFinTour1]
          );

          // ✅ Corriger aussi date_fin dans la table election
          await pool.execute(
            `UPDATE election SET date_fin = ? WHERE id_election = ?`,
            [dateFinTour1, elec.id_election]
          );

          console.log(
            `📋 [CRON] Tour 1 créé pour élection ${elec.id_election},` +
            ` fin prévue : ${dateFinTour1}`
          );

          // ✅ Ne pas dépouiller tout de suite — le tour vient d'ouvrir
          continue;
        }

        // ── Tour existant : vérifier si date_fin est dépassée ──────────────
        const dateFin    = new Date(elec.date_fin);
        const maintenant = new Date();

        if (dateFin > maintenant) {
          // Tour encore en cours, rien à faire
          continue;
        }

        // ── date_fin dépassée → dépouiller ────────────────────────────────
