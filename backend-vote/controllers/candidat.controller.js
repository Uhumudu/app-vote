...
export const createCandidat = async (req, res) => {
  try {
    ...
    // ✅ CORRIGÉ : bloque EN_COURS et TERMINEE
    if (elecRows[0].statut === "EN_COURS" || elecRows[0].statut === "TERMINEE") {
      return res.status(403).json({
        message: "Impossible d'ajouter un candidat à une élection en cours ou terminée"
      });
    }
...
export const updateCandidat = async (req, res) => {
  try {
    ...
    if (rows[0].statut === "EN_COURS" || rows[0].statut === "TERMINEE") {
      return res.status(403).json({
        message: "Impossible de modifier un candidat pendant ou après l'élection"
      });
    }
...
export const deleteCandidat = async (req, res) => {
  try {
    ...
    if (rows[0].statut === "EN_COURS" || rows[0].statut === "TERMINEE") {
      return res.status(403).json({
        message: "Impossible de supprimer un candidat pendant ou après l'élection"
      });
    }
...
export const importCandidatsCSV = async (req, res) => {
  try {
    ...
    if (elecRows[0].statut === "EN_COURS" || elecRows[0].statut === "TERMINEE") {
      return res.status(403).json({
        message: "Impossible d'importer des candidats pendant ou après l'élection"
      });
    }
    ...
    for (const c of candidats) {
      try {
        ...
        await pool.execute(
          `INSERT INTO candidat (nom, parti, age, photo, election_id, liste_id) VALUES (?, ?, ?, ?, ?, ?)`,
          [c.nom.trim(), c.parti || null, c.age || null, c.photo || null, electionId, listeId]
        );
...
export const deleteListe = async (req, res) => {
  try {
    ...
    // ✅ AJOUT : vérifier le statut de l'élection via la liste
    const [listeRows] = await pool.execute(
      `SELECT l.id_liste, e.statut, e.admin_id
       FROM liste l
       JOIN election e ON l.election_id = e.id_election
       WHERE l.id_liste = ?`,
      [listeId]
    );
    ...
    if (Number(listeRows[0].admin_id) !== Number(req.user.id)) {
      return res.status(403).json({ message: "Accès refusé" });
    }

    if (listeRows[0].statut === "EN_COURS" || listeRows[0].statut === "TERMINEE") {
      return res.status(403).json({
        message: "Impossible de supprimer une liste pendant ou après l'élection"
      });
    }
...
//     // ✅ FIX : conversion en nombre pour éviter le bug string vs int du JWT
//     if (Number(rows[0].admin_id) !== Number(req.user.id)) {
//       return res.status(403).json({ message: "Accès refusé" });
//     }
...
//         // ✅ CORRIGÉ : photo incluse dans l'INSERT
//         await pool.execute(
//           `INSERT INTO candidat (nom, parti, age, photo, election_id, liste_id) VALUES (?, ?, ?, ?, ?, ?)`,
//           [c.nom.trim(), c.parti || null, c.age || null, c.photo || null, electionId, listeId]
//         );
