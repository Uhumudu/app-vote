...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    // ── Nb électeurs inscrits (élections PRIVÉES uniquement) ─────────────────
    const [electeursPrivesCount] = await pool.execute(`
      SELECT COUNT(DISTINCT ee.electeur_id) AS total
      FROM electeur_election ee
      JOIN election e ON ee.election_id = e.id_election
      WHERE e.admin_id = ?
        AND e.visibilite = 'PRIVEE'
    `, [adminId]);

    // ── Nb votes privés (vote + vote_tour tour 1) ─────────────────────────────
    const [votesPrivesCount] = await pool.execute(`
      SELECT (
        SELECT COUNT(*)
        FROM vote v
        JOIN election e ON v.election_id = e.id_election
    ...
    // ── Nb votants uniques publics (téléphones distincts ayant payé) ──────────
    const [voteursPublicsCount] = await pool.execute(`
      SELECT COUNT(DISTINCT vp.telephone_electeur) AS total_votants,
             COALESCE(SUM(vp.nb_voix), 0)          AS total_voix
      FROM vote_public vp
      JOIN election e ON vp.election_id = e.id_election
      WHERE e.admin_id = ?
        AND vp.statut_paiement = 'PAYÉ'
    ...
    // ── Taux de participation combiné ─────────────────────────────────────────
    // Privé  : votes exprimés / électeurs inscrits
    // Public : votants uniques (téléphones) / total voix exprimées
    //          → on mesure l'engagement : nb votants uniques vs nb_voix totales
    // Global : (votes_prives + votants_uniques_publics) / (electeurs_prives + voix_publiques)
    const nbVotesPrives     = votesPrivesCount[0].total;
    const nbElecteursPrives = electeursPrivesCount[0].total;
    const nbVoteursPublics  = Number(voteursPublicsCount[0].total_votants);
    const nbVoixPubliques   = Number(voteursPublicsCount[0].total_voix);

    // Dénominateur : électeurs inscrits (privé) + nb de voix exprimées (public)
    // Numérateur   : votes exprimés (privé)     + votants uniques (public)
    const denominateur = nbElecteursPrives + nbVoixPubliques;
    const numerateur   = nbVotesPrives     + nbVoteursPublics;

    const tauxParticipation = denominateur > 0
      ? Math.round((numerateur / denominateur) * 100 * 100) / 100
      : 0;
