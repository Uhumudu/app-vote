...
// ✅ FIX : photo_url maintenant enregistrée dans la table election
// ─────────────────────────────────────────────────────────────────────
async function creerElectionEnDB(donnees, adminId) {
  const {
    ...
    nbSieges,
    visibilite,
    photoUrl,        // ✅ FIX : récupéré depuis donnees_election
  } = donnees;
  ...
  // ✅ FIX : visibilite et photo_url insérés dans la requête SQL
  const visibiliteDB = visibilite === "PUBLIQUE" ? "PUBLIQUE" : "PRIVEE";

  const [result] = await pool.execute(
    `INSERT INTO election
      (titre, description, date_debut, date_fin, statut, visibilite, admin_id, tour_courant, nb_sieges, duree_tour_minutes, photo_url)
     VALUES (?, ?, ?, ?, 'EN_ATTENTE', ?, ?, 1, ?, ?, ?)`,
    [
      titre,
      description,
      dateDebut,
      dateFin,
      visibiliteDB,                              // ✅ visibilite enregistrée
      adminId,
      isListe ? (nbSieges || 0) : 0,
      isListe ? (dureeTourMinutes || 1440) : 1440,
      photoUrl || null,                          // ✅ photo_url enregistrée
    ]
  );
