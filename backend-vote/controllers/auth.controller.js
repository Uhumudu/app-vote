...
// ✅ Helper : normalise n'importe quelle valeur de visibilité vers l'enum DB
// DB enum : 'PUBLIQUE' | 'PRIVEE'
// Accepte : "PUBLIQUE" → "PUBLIQUE", tout le reste → "PRIVEE"
...
export const registerAndCreateElection = async (req, res) => {
  try {
    ...
    // ✅ FIX : utilise le helper robuste qui accepte "PUBLIQUE", "PRIVEE", "PRIVE", etc.
    ...
    const [electionResult] = await pool.execute(
      `INSERT INTO election
         (titre, description, date_debut, date_fin, statut, admin_id,
          duree_tour_minutes, nb_sieges, visibilite, photo_url)
       VALUES (?, ?, ?, ?, 'EN_ATTENTE', ?, ?, ?, ?, ?)`,
      [
        electionName,
        description || "",
        ...
        isListe ? duree : null,
        isListe ? siege : null,
        modeVisibilite,   // ✅ toujours "PUBLIQUE" ou "PRIVEE"
        photoUrl || null,
      ]
    );
...
//     //  Normalisation de la visibilité — accepte PUBLIQUE ou PRIVE
...
//     // 2. Créer l'élection avec visibilite 
//     const [electionResult] = await pool.execute(
//       `INSERT INTO election
//          (titre, description, date_debut, date_fin, statut, admin_id,
//           duree_tour_minutes, nb_sieges, visibilite, photo_url)
