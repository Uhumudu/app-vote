...
// // ================================
// // 📝 REGISTER + CREATE ELECTION
// // POST /api/auth/register-and-create-election
// // ================================
// export const registerAndCreateElection = async (req, res) => {
//   try {
//     const {
...
//     // Pour non-LISTE : date_fin obligatoire
//     if (!isListe && !endDate) {
//       return res.status(400).json({ message: "La date de fin est obligatoire." });
//     }
...
//     // Pour LISTE : date_fin = startDate + dureeTourMinutes (fin du premier tour)
//     // Pour autres : date_fin fournie par le formulaire
//     const duree = parseInt(dureeTourMinutes) || 1440;
//     const end   = isListe
//       ? formatDate(new Date(new Date(startDate).getTime() + duree * 60000))
//       : formatDate(endDate);

//     console.log("Start:", start, "| End:", end, "| Type:", electionType, "| Durée tour:", isListe ? duree : "N/A");

//     // 1. Créer l'utilisateur avec rôle ADMIN_ELECTION_PENDING
//     const hashedPassword = await bcrypt.hash(motDePasse, 10);
//     const [userResult] = await pool.execute(
//       `INSERT INTO utilisateur (nom, prenom, email, mot_de_passe, role, actif)
//        VALUES (?, ?, ?, ?, 'ADMIN_ELECTION_PENDING', 1)`,
...
//     // 2. Créer l'élection avec duree_tour_minutes si scrutin LISTE
//     const [electionResult] = await pool.execute(
//       `INSERT INTO election (titre, description, date_debut, date_fin, statut, admin_id, duree_tour_minutes)
//        VALUES (?, ?, ?, ?, 'EN_ATTENTE', ?, ?)`,
//       [
//         electionName,
//         description || "",
