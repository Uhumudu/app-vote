...
export const getElecteursByElection = async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute(`
      SELECT u.id, u.nom, u.prenom, u.email, u.actif, ee.a_vote
      FROM utilisateur u
...
export const addElecteur = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "Électeur ajouté", userId, mot_de_passe: rawPassword, emailEnvoye: true });
  } catch (error) {
...
export const importElecteursCSV = async (req, res) => {
  try {
    ...
    for (const e of electeurs) {
      try {
        ...
        const [existing] = await pool.execute(
          `SELECT id FROM utilisateur WHERE email = ?`, [e.email.trim()]
        );
        ...
        if (alreadyIn.length === 0) {
          ...
          inserted++;
          results.push({ nom: e.nom, prenom: e.prenom, email: e.email, mot_de_passe: rawPassword });

          try {
            await sendCredentialsEmail({
              email: e.email.trim(), nom: e.nom.trim(),
              prenom: e.prenom.trim(), motDePasse: rawPassword, titreElection,
            });
          } catch (mailErr) {
    ...
    res.status(201).json({ message: `${inserted} électeur(s) importé(s)`, inserted, electeurs: results, errors });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
...
export const resetPasswordElecteur = async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute(
      `SELECT nom, prenom, email FROM utilisateur WHERE id = ?`, [electeurId]
    );

    if (rows.length > 0) {
      const { nom, prenom, email } = rows[0];
      try {
        await sendCredentialsEmail({ email, nom, prenom, motDePasse: rawPassword, titreElection: "votre élection" });
      } catch (mailErr) {
        console.error(`⚠️ Email reset non envoyé à ${email} :`, mailErr.message);
      }
    }

    res.json({ message: "Mot de passe réinitialisé", mot_de_passe: rawPassword });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
...
// ✅ CORRIGÉ : liste_id → id_liste (nom réel de la colonne dans la table liste)
...
export const voterElection = async (req, res) => {
  try {
    ...
    const typeScrutin = elecRows[0].type;

    // 2. Vérifier inscription + déjà voté
    const [alreadyVoted] = await pool.execute(
      `SELECT a_vote FROM electeur_election WHERE electeur_id = ? AND election_id = ?`,
      [electeurId, electionId]
    );
    ...
    // 3. Enregistrer le vote selon le type de scrutin
    if (typeScrutin === "BINOMINAL") {
      if (!candidat_ids || candidat_ids.length !== 2)
        return res.status(400).json({ message: "Vous devez sélectionner exactement 2 candidats" });

      await pool.execute(
        `INSERT INTO vote (electeur_id, election_id, candidat_id, candidat2_id)
         VALUES (?, ?, ?, ?)`,
        [electeurId, electionId, candidat_ids[0], candidat_ids[1]]
      );
    } else {
      if (!candidat_ids || candidat_ids.length === 0)
        return res.status(400).json({ message: "Vous devez sélectionner un candidat" });

      await pool.execute(
        `INSERT INTO vote (electeur_id, election_id, candidat_id)
         VALUES (?, ?, ?)`,
        [electeurId, electionId, candidat_ids[0]]
      );
    }

    // 4. Marquer l'électeur comme ayant voté
    await pool.execute(
      `UPDATE electeur_election SET a_vote = 1 WHERE electeur_id = ? AND election_id = ?`,
      [electeurId, electionId]
    );
    ...
    console.error("❌ Erreur voterElection:", error.message);
...
//     // 3. Enregistrer le vote selon le type de scrutin
//     if (typeScrutin === "BINOMINAL") {
//       // ── BINOMINAL : validation stricte de 2 candidats ──────────────────
//       if (!candidat_ids || candidat_ids.length !== 2)
//         return res.status(400).json({ message: "Vous devez sélectionner exactement 2 candidats" });

//       // Un seul enregistrement avec candidat_id + candidat2_id
//       // (évite la contrainte UNIQUE sur electeur_id + election_id)
//       await pool.execute(
//         `INSERT INTO vote (electeur_id, election_id, candidat_id, candidat2_id)
//          VALUES (?, ?, ?, ?)`,
//         [electeurId, electionId, candidat_ids[0], candidat_ids[1]]
...
//   } catch (error) {
//     console.error("❌ Erreur voterElection:", error.message);
//     // Doublon (contrainte UNIQUE) — l'électeur a déjà voté
//     if (error.code === "ER_DUP_ENTRY")
//       return res.status(403).json({ message: "Vous avez déjà voté à cette élection" });
//     res.status(500).json({ error: error.message });
//   }
